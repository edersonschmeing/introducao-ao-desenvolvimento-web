...
router.get("/:id", async (req, res) => {
    ...
    try {
        // para obter um livro pode-se utilizar .select().where()
        const livro = await dbKnex("livros").select().where({ id });
        res.status(200).json(livro); // retorna statusCode ok e os dados
    } catch (error) {
        res.status(400).json({ msg: error.message }); // retorna status de erro e msg
    }
});
